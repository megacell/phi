...
CREATE TABLE waypoint_voronoi (id integer, geom geometry);
...
BEGIN
    FOR d IN SELECT density_id FROM orm_waypoint GROUP BY density_id LOOP
        ...
        INSERT INTO waypoint_voronoi  select * from voronoi('waypoint_grouping', 'location') as (id integer, geom geometry);
    END LOOP;
    RETURN success;
END;
$$ LANGUAGE plpgsql;
...
FROM (
    ...
    WHERE ST_Contains(v.geom, w.location)
    ) AS sq
WHERE w_id = w.id;
