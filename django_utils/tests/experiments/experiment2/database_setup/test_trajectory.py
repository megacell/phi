...
class TestTrajectory(unittest.TestCase):
    def setUp(self):
        ...
        self.length_cache = {k:id_to_geometry[k].length for k in range(4)}

    def test_length(self):
        t = Trajectory([1,2], None, self.id_to_geometry, self.length_cache)
        self.assertAlmostEqual(t.length(), 2, delta=.0001)

    def test_length_complex(self):
        t = Trajectory([3], None, self.id_to_geometry, self.length_cache)
        self.assertAlmostEqual(t.length(), 6, delta=.0001)

    def test_match_percent(self):
        t1 = Trajectory([0,1], None, self.id_to_geometry, self.length_cache)
        t2 = Trajectory([0], None, self.id_to_geometry, self.length_cache)
        self.assertAlmostEqual(t1.match_percent(t2), 0.5, delta=.0001)

    def test_match_percent_no_overlapping_segments(self):
        t1 = Trajectory([0], None, self.id_to_geometry, self.length_cache)
        t2 = Trajectory([1], None, self.id_to_geometry, self.length_cache)
